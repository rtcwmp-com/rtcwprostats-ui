...
export const createLeadersApi = (agent: AxiosInstance) => {
  ...
  return {
    GetLeaders: async (params: Object) => {
		  ...
		  item_limit ? item_limit_string="" : item_limit_string="/limit/" + item_limit;
			  
		  return agent.get<LeaderItem[]>(`/leaders/${category}/region/${region}/type/${match_type}${item_limit_string}`).then(pickData); 
    },
  };
};
